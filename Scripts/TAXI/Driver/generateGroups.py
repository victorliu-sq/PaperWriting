...
def process_taxi_data(file_path):
    ...
    grouped_df = df.groupby('group_id')['tripID'].apply(lambda x: ','.join(map(str, x))).reset_index()
    grouped_df.columns = ["Average Score Bin", "Driver IDs"]

    # Save to CSV without truncating any data
    # grouped_df.to_csv('grouped_tripIDs.csv', index=False)
    grouped_df.to_csv("inverse_expanded_driver_groups_by_average_score.csv", index=False, quoting=csv.QUOTE_NONNUMERIC)
    print("Grouped trip IDs have been written to 'grouped_tripIDs.csv'.")

if __name__ == "__main__":
    # Provide the path to your Parquet file
    file_path = '/Users/jiaxinliu/Desktop/FlashSMPEvaluation/DataSets/TAXI/Driver/yellow_tripdata_2024-01.parquet'
    process_taxi_data(file_path)
